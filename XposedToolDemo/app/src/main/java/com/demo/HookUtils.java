        ...
        if(v3 >= 1) {
            ...
            if(TextUtils.isEmpty(v3_1)) {
                ...
                if(!TextUtils.isEmpty(v3_2) && v5 != -1 && v7 != -1 && v8 != -1 && v10 != -1 && v11 != -1 && v12 != -1 && v14 != -1 && v4_1 != -1 && v16 != -1) {
                    ...
                    MyLog.e("准备sub .....   console.info log  ");
                    return v3_1.substring(0, v4_1) + (" console.info(\"my_error 1111 \",  this.bottle.status);  console.info(\"my_test 1111 \",  this.bottle.status);  if(\"prepare\" == this.bottle.status) {var i=(Date.now()-this.mouseDownTime)/1e3;var vz=Math.min(i*" + v13 + ".velocityZIncrement,150);vz=+vz.toFixed(2);var vy=Math.min(" + v13 + ".velocityY+i*" + v13 + ".velocityYIncrement,180);vy=+vy.toFixed(2);this.vectorHelperOne.set(this.nextBlock.obj.position.x-this.bottle.obj.position.x,this.nextBlock.obj.position.z-this.bottle.obj.position.z);this.vectorHelperOne.x=+this.vectorHelperOne.x.toFixed(2);this.vectorHelperOne.y=+this.vectorHelperOne.y.toFixed(2);var r=vy/" + v6 + ".gravity*2;var n=this.bottle.obj.position.y.toFixed(2);var a=" + v15 + ".height/2-n;r=+(r-=+((-vy+Math.sqrt(Math.pow(vy,2)-2*" + v6 + ".gravity*a))/-" + v6 + ".gravity).toFixed(2)).toFixed(2);var s=[];this.vectorHelperTwo.set(this.bottle.obj.position.x,this.bottle.obj.position.z);var l=this.vectorHelperOne.setLength(vz * r);this.vectorHelperTwo.add(l);s.push(+this.vectorHelperTwo.x.toFixed(2),+this.vectorHelperTwo.y.toFixed(2));this.helperArrow.position.set(s[0],this.nextBlock.obj.position.y+" + v15 + ".height/2+.15,s[1]);var array=this.helperLine.geometry.attributes.position.array;array[0]=this.currentBlock.obj.position.x;array[1]=this.currentBlock.obj.position.y+" + v15 + ".height/2+.15;array[2]=this.currentBlock.obj.position.z;array[3]=s[0];array[4]=this.nextBlock.obj.position.y+" + v15 + ".height/2+.15;array[5]=s[1];this.helperLine.geometry.computeBoundingSphere();this.helperLine.geometry.attributes.position.needsUpdate=true;}else{this.helperArrow.position.set(-300,0,0);var array=this.helperLine.geometry.attributes.position.array;array[0]=-300;array[1]=0;array[2]=0;array[3]=-500;array[4]=0;array[5]=0;this.helperLine.geometry.computeBoundingSphere();this.helperLine.geometry.attributes.position.needsUpdate = true;}") + v3_1.substring(v4_1, v3_1.length());
                }else {
